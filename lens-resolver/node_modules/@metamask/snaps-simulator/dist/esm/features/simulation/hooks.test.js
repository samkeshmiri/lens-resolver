...
import { getSnapState, showDialog, showInAppNotification, showNativeNotification, updateSnapState } from "./hooks";
import { closeUserInterface, getSnapName, getSnapStateSelector, resolveUserInterface, setSnapState, showUserInterface } from "./slice";
        ...
        return _ts_generator(this, function(_state) {
            switch(_state.label){
                case 0:
                    return [
                        4,
                        expectSaga(showDialog, snapId, DialogType.Alert, text("foo")).withState({
                            simulation: {
                                manifest: MOCK_MANIFEST_FILE
                            }
                        }).select(getSnapName).put(showUserInterface({
                            snapId: snapId,
                            snapName: "@metamask/example-snap",
                            type: DialogType.Alert,
                            node: text("foo")
                        })).dispatch(resolveUserInterface("foo")).put(closeUserInterface()).returns("foo").silentRun()
                    ];
                case 1:
                    _state.sent();
                    return [
        ...
        return _ts_generator(this, function(_state) {
            switch(_state.label){
                case 0:
                    return [
                        4,
                        expectSaga(showNativeNotification, snapId, {
                            type: "native",
                            message: "foo"
                        }).withState({
                            simulation: {
        ...
        return _ts_generator(this, function(_state) {
            switch(_state.label){
                case 0:
                    return [
                        4,
                        expectSaga(showInAppNotification, snapId, {
                            type: "inApp",
                            message: "foo"
                        }).withState({
                            simulation: {
        ...
        return _ts_generator(this, function(_state) {
            switch(_state.label){
                case 0:
                    return [
                        4,
                        expectSaga(updateSnapState, snapId, "foo").withState({
                            simulation: {
                                snapState: null
                            }
                        }).put(setSnapState("foo")).silentRun()
                    ];
                case 1:
                    _state.sent();
                    return [
        ...
        return _ts_generator(this, function(_state) {
            switch(_state.label){
                case 0:
                    return [
                        4,
                        expectSaga(getSnapState, snapId).withState({
                            simulation: {
                                snapState: "foo"
                            }
                        }).select(getSnapStateSelector).returns("foo").silentRun()
                    ];
                case 1:
                    _state.sent();
                    return [
